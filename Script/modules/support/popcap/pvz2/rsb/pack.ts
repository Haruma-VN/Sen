    ...
    export function PackPopCapRSBBySimple(inDirectory: string, outFile: string, option: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Pack.Options): void {
        ...
        if (Fs.DirectoryExists(properties)) {
            ...
            Console.Print(
                ...
                Sen.Script.Modules.System.Default.Localization.GetString("execution_process").replace(/\{\}/g, `${json_count} JSONs`)
            );
        }
        ...
        if (manifest_group !== -1) {
            ...
            Console.Print(
                ...
                Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                    /\{\}/g,
                    Sen.Script.Modules.System.Default.Localization.GetString("finish_rsg_pack").replace(
                        /\{\}/g,
                        Path.Parse(manifestgroup_save).name_without_extension
                    )
                )
        ...
        if (packages !== -1) {
            ...
            Console.Print(
                ...
                Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                    ...
                    Sen.Script.Modules.System.Default.Localization.GetString("finish_rsg_pack").replace(
                        /\{\}/g,
                        Path.Parse(packages_save).name_without_extension
                    )
                )
            );
        }
        PvZ2Shell.RSBPack(inDirectory, outFile, manifest);
        Fs.DeleteFile(packages_save);
        Fs.DeleteFile(manifestgroup_save);
        return;
    }
}
