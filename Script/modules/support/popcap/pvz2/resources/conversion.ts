namespace Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Conversion {
    ...
    export abstract class CheckOfficialResources {
        ...
        protected static CheckOfficial<Template extends Resources_Group_Structure_Template>(resources_group: Template, file_path: string): resources_group is Template {
    ...
    export class UnofficialResourceConversion extends CheckOfficialResources {
        ...
        private static ConvertOfficialSubgroupFile<Template extends Resource_Structure_Template, Value extends subgroup_children>(sub_resource_data: Template, file_path: string): Value {
            this.CheckWholeData(sub_resource_data, file_path);
        ...
        private static ConvertOfficialSubgroupContainsAtlas<Template extends Resource_Structure_Template>(sub_resource_data: Template, file_path: string): sprite_data {
                    ...
                    for (let k_index: number = 0; k_index < subgroup_list[j_index].length; ++k_index) {
                        res_json.groups[current_subgroup_name].subgroup[subgroup_list[j_index][k_index].name] =
                            subgroup_list[j_index][k_index].image !== null
                                ? Object.values(this.ConvertOfficialSubgroupContainsAtlas(resources_group.groups.filter((res) => res.id === subgroup_list[j_index][k_index].name)[0] as any, file_path))[0]
                                : (Object.values(this.ConvertOfficialSubgroupFile(resources_group.groups.filter((res) => res.id === subgroup_list[j_index][k_index].name)[0] as any, file_path) as any)[0] as any);
                    }
                }
            }
    ...
    export class ConvertToOfficial extends CheckOfficialResources {
        ...
        private static GenerateOfficialComposite<Template extends subgroup_children>(subgroup: Template, subgroup_parent: string, file_path: string): composite_object {
        ...
        public static DoAllProcess<Res_Json_Template extends res_json, Resource_json_Template extends Resources_Group_Structure_Template>(res_json: Res_Json_Template, file_path: string): Resource_json_Template {
            ...
            for (let index: number = 0; index < subgroups_key.length; ++index) {
                if (res_json.groups[subgroups_key[index]].is_composite === true) {
                    ...
                    (resources_json as any).groups.push(this.GenerateOfficialComposite<subgroup_children>(res_json.groups[subgroups_key[index]].subgroup, subgroups_key[index], file_path));
