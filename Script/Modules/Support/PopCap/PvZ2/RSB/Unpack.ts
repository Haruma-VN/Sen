namespace Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack {
    ...
    export function ConvertToManifest(
        information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo
    ...
    export function UnpackPopCapOfficialRSBBySimple(
        ...
        const manifest_additional_information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo =
        ...
        if (manifest_group) {
            Sen.Shell.PvZ2Shell.RSGUnpack(
                Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${outDirectory}`, `packet`, `${manifest_group}.rsg`)),
                Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${outDirectory}`, `resource`)),
                false
            );
            ...
            // PVZ2C CONFIG.RSB
                ...
                Sen.Shell.FileSystem.FileExists(
                    Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${outDirectory}`, `resource`, `PROPERTIES`, `RESOURCESCONFIG.RTON`))
                )
            ) {
            ...
            if (resources_rton_path !== "") {
                const resources_json_path: string = Sen.Shell.Path.Resolve(
                    Sen.Shell.Path.Join(
                        `${Sen.Shell.Path.Dirname(resources_rton_path)}`,
                        `${Sen.Shell.Path.Parse(resources_rton_path).name_without_extension}.json`
                    )
                );
        ...
        if (packages) {
            ...
            Sen.Shell.PvZ2Shell.RSGUnpack(
                Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${outDirectory}`, `packet`, `${packages}.rsg`)),
    ...
    export function UnpackAbnormalRSBByLooseConstraints(inRSB: string, outDirectory: string): Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo {
        const manifest_json: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo = Sen.Shell.PvZ2Shell.RSBUnpackByLooseConstraints(
            inRSB,
            outDirectory
        );
        Sen.Script.Modules.FileSystem.Json.WriteJson<Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation>(
    ...
    export function ConvertToSimplifiedManifest(
        information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo
    ): Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.SimplifiedManifest<string> {
