namespace Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Resource {
    ...
    export function UnpackAllPopCapRSGs(rsb_dir: string, out_dir: string): void {
        ...
        const rsgs: Array<string> = Sen.Shell.FileSystem.ReadDirectory(
            packet_directory,
    ...
    export function UnpackAllPopCapRSGDataInsideUnpackDirectory(
        rsb_unpack_option: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Resource.RSBConvertOption,
        out_dir: string
    ): void {
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("unpacking_all_rsgs")
            )
        );
        ...
        const popcap_resource_group_resources_rton_input_destination: string = Sen.Shell.Path.Resolve(
            Sen.Shell.Path.Join(`${rsb_unpack_option.rsb_unpack_dir}`, `PROPERTIES`, `RESOURCES.RTON`)
        );
        const popcap_resource_group_resources_json_output_destination: string = Sen.Shell.Path.Resolve(
            Sen.Shell.Path.Join(`${out_dir}`, `PROPERTIES`, `RESOURCES.JSON`)
        );
        ...
        const res_json: res_json = Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Conversion.UnofficialResourceConversion.DoAllProcess<
            Resources_Group_Structure_Template,
            res_json
        >(
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("converted_resources_json_to_res_json")
            )
        );
        ...
        const manifest_deserialize: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation =
        ...
        compositeshell.forEach((parent: string) => {
            ...
            subgroup.forEach((subgroup_children: string) => {
                manifest_deserialize.group[parent].subgroup[subgroup_children].packet_info.res.forEach(
                    (worker: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ResInfo) => {
                        ...
                        const resource_popcap_texture_format_information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.PtxProperty =
                            worker.ptx_property as Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.PtxProperty;
                        const resource_popcap_texture_dimension_information: Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo =
                            worker.ptx_info as Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo;
                        const input_path: string = Sen.Shell.Path.Resolve(
                            Sen.Shell.Path.Join(`${rsb_unpack_option.rsb_unpack_dir}`, `${resource_unpack_path}`)
                        );
                        const output_path: string = Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${out_dir}`, `${resource_unpack_path}`));
                        if (
                            resource_popcap_texture_format_information !== null &&
                            resource_popcap_texture_dimension_information !== null &&
                            rsb_unpack_option.extractAtlas
                        ) {
                            Sen.Script.Modules.Support.PopCap.PvZ2.Texture.Encode.DecodePopCapPTX(
                                ...
                                Sen.Script.Modules.Support.PopCap.PvZ2.Texture.Encode.SwapTextureFormatInformation(
                                    architecture[`${resource_popcap_texture_format_information.format}`]
                                )
                            );
                            ...
                            if (rsb_unpack_option.pam_to_flash_animation) {
                                const destination: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Resource.FlashAnimationContainer = {
                                    filepath: Sen.Shell.Path.Resolve(
                                        Sen.Script.Modules.Support.PopCap.PvZ2.Animation.PopCapAnimationJsonToAnimateAdobeFlashAnimation(
                                            output_pam_json,
                                            output_pam_json.replace(/((\.pam.json))?$/i, `.xfl`),
                            ...
                            Sen.Script.Modules.Support.WWise.Soundbank.Encode.WWiseSoundbankDecodeBySimple(
                                input_path,
                                output_path.replace(/((\.bnk))?$/i, `.soundbank`)
                            );
                        }
                    }
                );
    ...
    export function PackPopCapRSBInsideConvertDirectory(rsb_packing_option: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Resource.RSBPackOption): void {
        ...
        const manifest_deserialize: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation =
            Sen.Script.Modules.FileSystem.Json.ReadJson<Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation>(manifest);
        const converted_manifest_for_shell: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ManifestInfo =
            Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Pack.ConvertFromManifest(manifest_deserialize);
        ...
        converted_manifest_for_shell.group.forEach((group: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.GroupInfo) => {
            group.subgroup.forEach((subgroup: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.SubGroupInfo) => {
                Sen.Shell.PvZ2Shell.RSGPack(
                    `${unpack_directory}`,
                    Sen.Shell.Path.Resolve(Sen.Shell.Path.Join(`${packet_directory}`, `${subgroup.name_packet}.rsg`)),
                    subgroup.packet_info,
                    false
                );
            });
        });
