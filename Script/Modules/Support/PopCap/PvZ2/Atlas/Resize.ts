namespace Sen.Script.Modules.Support.PopCap.PvZ2.Atlas.Resize {
    ...
    export function ResizeAutomatically<T extends Record<"width" | "height", int>>(file_path: string, modify_number: float, output_path: string, dimension: T): void {
        Sen.Shell.DotNetBitmap.ResizeImage(BeautifyDimension(dimension.width * modify_number), BeautifyDimension(dimension.height * modify_number), file_path, output_path);
        return;
    }
    ...
    export class ResizePopCapSprite extends Sen.Script.Modules.Support.PopCap.PvZ2.Atlas.Pack.PackFromAtlasJson {
         ...
         */
        protected static RenamePathForAtlasJson(atlas_json: Sen.Script.Modules.Support.PopCap.PvZ2.Atlas.Split.AtlasJson, original: int, modified: int): Sen.Script.Modules.Support.PopCap.PvZ2.Atlas.Split.AtlasJson {
        ...
        public static DoAllResizeBasedOnAtlasJson(directory_path: string, original: int, modified: int, output_directory: string): void {
            if (Sen.Shell.FileSystem.DirectoryExists(output_directory)) {
            ...
            const calculate: float = Sen.Script.Modules.Support.PopCap.PvZ2.Atlas.Resize.DoCalculate(modified, original);
            for (const data of packable_datas) {
