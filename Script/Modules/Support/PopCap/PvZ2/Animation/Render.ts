namespace Sen.Script.Modules.Support.PopCap.PvZ2.Animation.Render {
    ...
    export function LoadAnimationSpirte(
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("all_sprites_loaded")
            )
        );
        for (let i = 0; i < pam_json.sprite.length; i++) {
            ...
            for (let k = 0; k < pam_json.sprite[i].frame![0].append!.length; k++) {
                ...
                if (pam_json.sprite[i].frame![0].append![k].sprite) {
                    ...
                    const image_name: string = use_media_images_by_path
                        ? pam_json.image[resource_index].name!.split("|")[0]
                        : pam_json.image[resource_index].name!.split("|")[1];
            ...
            Sen.Shell.Console.Printf(
                ...
                `      "${i + 1}": ${sprite_name} | source: ${source_sprite_list.join(`, `)}`
            );
        }
        Sen.Shell.Console.Print(
    ...
    export function InputSprite(sprites_to_disable: Array<int>, sprite_length: int): void {
        ...
        while (input != 0) {
            ...
            while (sprites_to_disable.includes(input)) {
                Sen.Shell.Console.Print(
                    ...
                    `${Sen.Script.Modules.System.Default.Localization.GetString("execution_failed").replace(
                        /\{\}/g,
                        Sen.Script.Modules.System.Default.Localization.GetString("sprite_already_selected")
                    )}`
                ),
                    (input = parseInt(Sen.Shell.Console.Input(Sen.Script.Modules.Platform.Constraints.ConsoleColor.Cyan)));
            }
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("command_executed_success")
            )
        );
        return;
    }
    ...
    export function LoadSetting(
        ...
        const method: 1 | 2 | 3 = Sen.Script.Modules.Support.PopCap.PvZ2.Argument.Input.InputArgument.InputInteger(
            ...
            {
                "1": [
                    Sen.Script.Modules.System.Default.Localization.GetString("enable_all_sprites"),
                    Sen.Script.Modules.System.Default.Localization.GetString("enable_all_sprites"),
                ],
                "2": [
                    Sen.Script.Modules.System.Default.Localization.GetString("disable_all_sprites"),
                    Sen.Script.Modules.System.Default.Localization.GetString("disable_all_sprites"),
                ],
                "3": [
                    Sen.Script.Modules.System.Default.Localization.GetString("choose_sprite_to_disable"),
                    Sen.Script.Modules.System.Default.Localization.GetString("choose_sprite_to_disable"),
                ],
            },
        ...
        switch (method) {
            case 1:
                Sen.Shell.Console.Print(
                    Sen.Script.Modules.Platform.Constraints.ConsoleColor.Green,
                    Sen.Script.Modules.System.Default.Localization.GetString("execution_loaded").replace(
                        /\{\}/g,
                        Sen.Script.Modules.System.Default.Localization.GetString("enable_all_sprites")
                    )
                );
            ...
            case 3:
                ...
                Sen.Shell.Console.Print(
                    Sen.Script.Modules.Platform.Constraints.ConsoleColor.Green,
                    `${Sen.Script.Modules.System.Default.Localization.GetString("animation_render_disable_sprite_method")}: ${sprite_selected}`
                );
                break;
        }
    ...
    export function GenerateImageSequence(file_input: string, out_folder: string): void {
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_argument").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("provide_sprite_directory")
            )
        );
        ...
        if (!Sen.Shell.FileSystem.DirectoryExists(media_path)) {
            throw new Sen.Script.Modules.Exceptions.MissingDirectory(
                `${Sen.Script.Modules.System.Default.Localization.GetString(`no_such_directory`).replace(/\{\}/g, Sen.Shell.Path.GetFileName(media_path))}`,
                media_path
            );
        }
        ...
        if (!Sen.Shell.FileSystem.FileExists(atlas_json_path)) {
            throw new Sen.Script.Modules.Exceptions.MissingFile(
                `${Sen.Script.Modules.System.Default.Localization.GetString(`no_such_file`).replace(/\{\}/g, Sen.Shell.Path.GetFileName(atlas_json_path))}`,
                atlas_json_path
            );
        }
