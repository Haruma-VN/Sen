namespace Sen.Script.Modules.Executable.PvZ2.AndroidRSBtoiOSRSB {
     ...
     * @returns void
     */
    export function ConvertStreamingWavetoGlobalData(
        ...
        const streaming_wave_packet_info: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBPacketInfo = Sen.Shell.PvZ2Shell.RSGUnpack(
            streaming_wave_path,
            `${streaming_wave_bundle}`,
            false
        );
        ...
        for (const res_index in streaming_wave_packet_info.res) {
            const wem_path: string = Sen.Shell.Path.Resolve(
                Sen.Shell.Path.Join(`${streaming_wave_bundle}`, `${streaming_wave_packet_info.res[res_index].path as string}`)
            );
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_finish").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("converted_streaming_wave_to_global_data")
            )
        );
        return;
    }
    ...
    export function ConvertStreamingWavetoGlobalDataforResouces(
        manifest_rsg_bundle_directory: string,
        subgroup_content_json: Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Official.official_subgroup_json
    ): void {
        ...
        Sen.Script.Modules.FileSystem.Json.WriteJson<resource_atlas_and_sprites>(global_data_res_path, deserialized_subgroup);
    ...
    export function UnpackRSBAndConvertPTX(file_in: string, bundle_directory: string, input_resolution: number, compress_ptx: boolean): string {
        ...
        const information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation =
            ...
            if (information.group[group].is_composite) {
                ...
                for (const subgroup of subgroups) {
                    if (subgroup.toUpperCase().endsWith("_COMMON")) continue;
                        ...
                        for (let k = 0; k < information.group[group].subgroup[subgroup].packet_info.res.length; k++) {
                            const packet_res_info: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.ResInfo =
                                information.group[group].subgroup[subgroup].packet_info.res[k];
                        ...
                        const rsg_packetinfo: Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PacketInfo =
                            information.group[group].subgroup[subgroup].packet_info;
                        ...
                        Sen.Shell.Console.Print(
                            ...
                            `${Sen.Script.Modules.System.Default.Localization.GetString("execution_finish").replace(
                                /\{\}/g,
                                Sen.Script.Modules.System.Default.Localization.GetString("converted_rsg").replace(/\{\}/g, `${subgroup}`)
                            )}`
                        );
                    } else {
                ...
                if (!check_contain_atlas)
                    throw new Sen.Script.Modules.Exceptions.MissingProperty(
                        Sen.Script.Modules.System.Default.Localization.GetString("does_not_contains_in_rsb").replace(
                            /\{\}/g,
                            `${atlas_res_bundle[input_resolution - 1]}`
                        ),
    ...
    export function RewriteResources(bundle_directory: string, manifest_packet_name: string, input_resolution: number): void {
        ...
        const manifest_packet_info: Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PacketInfo = Sen.Shell.PvZ2Shell.RSGUnpack(
            manifest_rsg_path,
            `${manifest_rsg_bundle_directory}`,
            false
        );
        ...
        const output_argument: string = Sen.Shell.Path.Resolve(
            Sen.Shell.Path.Join(`${Sen.Shell.Path.Dirname(res_argument)}`, `${Sen.Shell.Path.Parse(res_argument).name_without_extension}.res`)
        );
        ...
        Sen.Script.Modules.FileSystem.Json.WriteJson<Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Official.official_subgroup_json>(
            content_json_path,
            subgroup_content_json
        );
    ...
    export function Evaluate(): void {
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_argument").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("input_rsb_path")
            )
        );
        const file_in: string = Sen.Script.Modules.Interface.Arguments.InputPath("file");
        const bundle_directory: string = Sen.Shell.Path.Resolve(
            `${Sen.Shell.Path.Join(`${Sen.Shell.Path.Dirname(file_in)}`, `${Sen.Shell.Path.Parse(file_in).name_without_extension}.bundle`)}`
        );
        Sen.Script.Modules.Interface.Arguments.ArgumentPrint(bundle_directory, "file");
        const output_file: string = Sen.Shell.Path.Resolve(
            `${Sen.Shell.Path.Dirname(file_in)}/${Sen.Shell.Path.Parse(file_in).name_without_extension}.converted.rsb`
        );
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_argument").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("popcap_ptx_encode")
            )
        );
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_argument").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("popcap_texture_resolution")
            )
        );
        Sen.Shell.Console.Printf(
            Sen.Script.Modules.Platform.Constraints.ConsoleColor.White,
            `      1. ${Sen.Script.Modules.System.Default.Localization.GetString("popcap_res_1536")}`
        );
        Sen.Shell.Console.Printf(
            Sen.Script.Modules.Platform.Constraints.ConsoleColor.White,
            `      2. ${Sen.Script.Modules.System.Default.Localization.GetString("popcap_res_768")}`
        );
        ...
        Sen.Shell.Console.Print(
            ...
            Sen.Script.Modules.System.Default.Localization.GetString("execution_status").replace(
                /\{\}/g,
                Sen.Script.Modules.System.Default.Localization.GetString("rsg_list")
            )
        );
        const manifest_packet_name: string = Sen.Script.Modules.Executable.PvZ2.AndroidRSBtoiOSRSB.UnpackRSBAndConvertPTX(
            file_in,
            bundle_directory,
            input_resolution,
            compress_ptx
