namespace Sen.Script.Modules.Executable.PvZ2.AndroidRSBtoiOSRSB {
    ...
    export function ConvertFormat(bundle_directory: string, atlas_name: string, ptx_info: Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo, atlas_format: number, compress_ptx: boolean): number {
        ...
        if (atlas_format === 0) {
            Sen.Shell.TextureHandler.Create_RGBA8888_Decode(ptx_path, png_path, (ptx_info as Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo).width as number, (ptx_info as Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo).height as number);
        } else if (atlas_format === 147) {
            Sen.Shell.TextureHandler.Create_ETC1_RGB_A8_Decode(ptx_path, png_path, (ptx_info as Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo).width as number, (ptx_info as Sen.Script.Modules.Support.PopCap.PvZ2.RSG.Encode.PtxInfo).height as number);
        }
    ...
    export function UnpackRSBAndConvertPTX(file_in: string, bundle_directory: string, input_resolution: number, compress_ptx: boolean): string {
        ...
        const information: Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation = Sen.Script.Modules.FileSystem.Json.ReadJson<Sen.Script.Modules.Support.PopCap.PvZ2.RSB.Unpack.RSBManifestInformation>(Sen.Shell.Path.Join(`${bundle_directory}`, `manifest.json`));
                ...
                for (const subgroup of subgroups) {
                    if (subgroup.toUpperCase().endsWith("_COMMON")) continue;
                        ...
                        for (let k = 0; k < information.group[group].subgroup[subgroup].packet_info.res.length; k++) {
                            ...
                            information.group[group].subgroup[subgroup].packet_info.res[k].ptx_property!.format = ConvertFormat(bundle_directory, (packet_res_info.path as Array<string>).join("\\"), packet_res_info.ptx_info!, packet_res_info.ptx_property!.format, compress_ptx);
                        }
    ...
    export function RewriteResources(bundle_directory: string, manifest_packet_name: string, input_resolution: number): void {
        ...
        const subgroup_content_json: Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Official.official_subgroup_json = Sen.Script.Modules.FileSystem.Json.ReadJson<Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Official.official_subgroup_json>(content_json_path);
        ...
        Sen.Script.Modules.FileSystem.Json.WriteJson<Sen.Script.Modules.Support.PopCap.PvZ2.Resources.Official.official_subgroup_json>(content_json_path, subgroup_content_json);
