...
namespace Sen.Shell.Modules.Support.PVZ.Reanim
{
    ...
    public class Reanim_RawXML
    {
            ...
            using (StreamWriter sw = new StreamWriter(outFile, false))
            {
                ...
                for (int i = 0; i < trackNumber; i++)
                {
                    ...
                    for (int j = 0; j < transformsNumber; j++)
                    {
                        ...
                        if (k.resource2 is not null)
                        {
                            sw.Write("<resource2>");
                            sw.Write(k.resource2);
                            sw.Write("</resource2>");
                        }
                        if (k.font is not null)
                        {
                            sw.Write("<font>");
                            sw.Write(k.font);
                            sw.Write("</font>");
                        }
                        if (k.text is not null)
                        {
                            sw.Write("<text>");
                            sw.Write(k.text);
                            sw.Write("</text>");
        ...
        public static Reanim Decode(string inFile)
        {
            ...
            {
                reanimInfo.do_scale = Convert.ToSByte(childlist[0]!.InnerText);
                    ...
                    foreach (XmlNode node in childchildlist[j]!.ChildNodes)
                    {
                        switch (node.Name)
                        {
                            ...
                            case "i":
                                ...
                                break;
                            case "resource":
                                k.resource = node.InnerText;
                                break;
                            case "i2":
                                k.i2 = node.InnerText;
                                break;
                            case "resource2":
                                k.resource2 = node.InnerText;
                                break;
                            case "font":
